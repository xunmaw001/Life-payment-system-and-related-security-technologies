            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑固话</li>
                    </ol>
                </div>
            </div>
        </div>
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <input id="guhuaMoney" name="guhuaMoney" class="form-control"
                                               onchange="guhuaMoneyChickValue(this)" placeholder="固话余额" value="0" readonly>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>固话编号所在地址</label>
                                        <input id="guhuaAddress" name="guhuaAddress" class="form-control"
                                               placeholder="固话编号所在地址">
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("guhua/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addguhua');
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                yonghuId: "required",
                guhuaNumber: "required",
                guhuaMoney: "required",
                guhuaAddress: "required",
            },
            messages: {
                yonghuId: "用户不能为空",
                guhuaNumber: "固话编号不能为空",
                guhuaMoney: "固话余额不能为空",
                guhuaAddress: "固话编号所在地址不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addguhua = window.sessionStorage.getItem("addguhua");
        if (addguhua != null && addguhua != "" && addguhua != "null") {
            window.sessionStorage.removeItem('addguhua');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("guhua/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显

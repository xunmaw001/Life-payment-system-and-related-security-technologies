...
<body>
    ...
    <div class="modal fade" id="dianbiaoListModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <%-- 模态框标题--%>
                <div class="modal-header">
                    <h5 class="modal-title" id="dianbiaoListModalTitle">列表</h5>
                ...
                <div class="modal-body">
                    <div class="col-sm-12">
                        ...
                        <%-- 详情表 --%>
                        <input type="hidden" id="dianbiaoListFlag">
                        <label class="col-sm-1">
                            <button onclick="addDianbiaoList()" type="button" class="btn btn-success 新增">添加</button>
                        </label>
                    </div>
                    <%-- 列表 --%>
                    <table id="dianbiaoTable" class="table table-bordered table-striped">
                        <thead>
                        <tr>
                            <th >缴费前余额</th>
                            <th >缴费金额(元)</th>
                        ...
                        <tbody id="dianbiaoListTableTbody">
                        </tbody>
                    </table>
                </div>
                <%-- 模态框按钮 --%>
                <div class="modal-footer">
                    ...
                    <button type="button" onclick="submitDianbiaoList()" class="btn btn-primary">提交</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <%--  列表查看 --%>
    <div class="modal fade" id="showDianbiaoListModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <%-- 模态框标题--%>
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <%--<div class="col-sm-12">
                        &lt;%&ndash; 当前表 &ndash;%&gt;
                        <label class="col-sm-1">
                            <button onclick="addShowDianbiaoList()" type="button" class="btn btn-success 新增">添加</button>
                        </label>
                    </div>--%>
                    <table id="inOutTableList" class="table table-bordered table-striped">
                        ...
                        <tbody id="showDianbiaoListTableTbody">
                        </tbody>
                    </table>
                </div>
                <%-- 模态框按钮 --%>
                            ...
                            <div class="table-responsive mb-3">
                                <div class="col-sm-12">
                                    ...
                                    <div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
                                        ...
                                        <%--<button onclick="outDianbiaoList()" type="button" class="btn btn-success 新增">出出出出出出出出出出出出出出出</button>--%>
...
// 当前表关联
    ...
    //查询当前表搜索条件所有列表
        ...
        //出出出出出出出出出出出出出出出
        function outDianbiaoList() {
            document.getElementById("dianbiaoListModalTitle").innerHTML = "出出出出出出出出出出出出出出出列表";
            $("#dianbiaoListFlag").val(2);
            $('#dianbiaoListModal').modal('show');
        }
        //入入入入入入入入入入入入入入入
        function inDianbiaoList() {
            document.getElementById("dianbiaoListModalTitle").innerHTML = "入入入入入入入入入入入入入入入列表";
            $("#dianbiaoListFlag").val(1);
            $('#dianbiaoListModal').modal('show');

        }

        // 模态框打开
        $('#dianbiaoListModal').on('show.bs.modal', function () {
        ...
        // 模态框关闭
        $('#dianbiaoListModal').on('hide.bs.modal', function () {
            $("#dianbiaoListModalTitle").val("")
            $("#dianbiaoListFlag").val(0);
            $("#dianbiaoName").val("");
            $("#dianbiaoListTableTbody").html("");
            // 当前表
            $("#yonghuSelect").empty();
            // 详情表
            $(".selectpicker" ).selectpicker('refresh');
        ...
        // 模态框的添加按钮
        function addDianbiaoList() {
        }

        // 添加电表缴费记录到待提交列表
        function setDianbiaoListDataRow(item) {
            //创建行 
            var row = document.createElement('tr');
                //缴费前余额
            ...
            deleteBtn.setAttribute("type", "button");
            deleteBtn.setAttribute("class", "btn btn-danger btn-sm 删除");
            deleteBtn.setAttribute("onclick","removeShowDianbiaoOne("+item.id+")");
            deleteBtn.innerHTML = "删除";
            btnGroup.appendChild(deleteBtn);
        ...
        function chickDianbiaoNumber(e,id){
            var this_val = e.value || 0;
            if(this_val == 0){
                e.value = "";
                alert("数量为0无意义");
        ...
        function submitDianbiaoList() {
            ...
            var flag = parseInt($("#dianbiaoListFlag").val());
            var urlParam=null;
            ...
            var msg=null;
            if(flag==1){
                urlParam="inDianbiaoList";
                msg="入入入入入入入入入入入入入入入成功";
            }else if(flag==2){
                urlParam="outDianbiaoList";
            ...
            httpJson("dianbiao/" + urlParam, "POST", {
                map : map,
        ...
        function getId(thisId) {
            ...
            id=thisId;
            $('#showDianbiaoListModal').modal('show');
        }

        //模态框打开的时候会执行这个里面的东西
        $('#showDianbiaoListModal').on('show.bs.modal', function () {
            //清理表格内容,防止重复回显
            $("#showDianbiaoListTableTbody").html("");
            if(id ==null){
                alert("获取当前列id错误,请联系管理员处理");
                $('#showDianbiaoListModal').modal('hide');
                return;
            }
            // 当前表
            yonghuSelect();//查询列表
            ...
            showDianbiaoList();//查询数据列表
        });
        //模态框关闭的时候会执行这个里面的东西
        $('#showDianbiaoListModal').on('hide.bs.modal', function () {
            $("#showDianbiaoListTableTbody").html("");
            id=null;
            ...
            http("dianbiaoList/page", "GET", {
                ...
                if(res.code == 0){
                    ...
                    for (var i = 0; i < list.length; i++) { //遍历一下表格数据  
                        var trow = setShowDianbiaoListDataRow(list[i]); //在 setShowDianbiaoListDataRow 中拼接数据,然后返回再追加到列表中
                        $('#showDianbiaoListTableTbody').append(trow);
                    }
                }else{
        ...
        // 列表回显
        function setShowDianbiaoListDataRow(item) {
            //创建行 
            var row = document.createElement('tr');
                //缴费前余额
            var dianbiaoListOldMoneyCell = document.createElement('td');
        ...
        // 删除
        function removeShowDianbiaoOne(id) {
            var mymessage = confirm("真的要删除吗？");
            if (mymessage == true) {
                httpJson("dianbiaoList/delete?id="+id, "GET",null, (res) => {
                    if(res.code == 0){
                    alert('删除成功');
                    showDianbiaoList();
                }else{
        ...
        //新增
        function addShowDianbiaoList() {
            httpJson("dianbiaoList/save", "POST", paramArray, (res) => {
                if(res.code == 0){
                    alert('新增成功');
                    showDianbiaoList();
                }else{
        ...
        function jiaofeiSubmit() {
            ...
            httpJson("dianbiao/jiaofei", "POST", {

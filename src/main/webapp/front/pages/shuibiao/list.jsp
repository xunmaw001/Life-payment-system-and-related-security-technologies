            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">水表展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="shuibiaoNumber" id="shuibiaoNumber" placeholder="水表编号" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">水表余额</label>
                          <div class="layui-input-inline">
                              <input type="text" name="shuibiaoMoneyStart" id="shuibiaoMoneyStart"  lay-skin="double"   placeholder="水表余额开始" autocomplete="off" class="layui-input">
                              -
                              <input type="text" name="shuibiaoMoneyend"   id="shuibiaoMoneyend"    lay-skin="double|shuibiaoMoneyend"   placeholder="水表余额结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">水表编号所在地址</label>
                          <div class="layui-input-inline">
                              <input type="text" name="shuibiaoAddress" id="shuibiaoAddress" placeholder="水表编号所在地址" autocomplete="off" class="layui-input">
                          </div>
                      </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 shuibiaoContent 字段去除无用字符 如果字段长度大于60  就截取到60
            shuibiaoContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        form.verify({
            ...
            ,shuibiaoMoneyend: function(value, item){ //value：表单的值、item：表单的DOM对象
                var shuibiaoMoneyStart = $("#shuibiaoMoneyStart").val();
                if(shuibiaoMoneyStart != null && shuibiaoMoneyStart != "" && shuibiaoMoneyStart != "null"){
                    var number = parseInt(value)- parseInt(shuibiaoMoneyStart);
                    if(number<0){
                        return '水表余额的结束值不能小于开始值';
                    }
                }
            }
        ...
        function pageList() {
            ...
            if (jquery('#shuibiaoNumber').val() != null && jquery('#shuibiaoNumber').val() != "" && jquery('#shuibiaoNumber').val() != "null" ) {
                param['shuibiaoNumber'] = jquery('#shuibiaoNumber').val();
            }
     
            if (jquery('#shuibiaoMoneyStart').val() != null && jquery('#shuibiaoMoneyStart').val() != "" && jquery('#shuibiaoMoneyStart').val() != "null" ) {
                param['shuibiaoMoneyStart'] = jquery('#shuibiaoMoneyStart').val();
            }
            if (jquery('#shuibiaoMoneyEnd').val() != null && jquery('#shuibiaoMoneyEnd').val() != "" && jquery('#shuibiaoMoneyEnd').val() != "null" ) {
                param['shuibiaoMoneyEnd'] = jquery('#shuibiaoMoneyEnd').val();
            }
     
            if (jquery('#shuibiaoAddress').val() != null && jquery('#shuibiaoAddress').val() != "" && jquery('#shuibiaoAddress').val() != "null" ) {
                param['shuibiaoAddress'] = jquery('#shuibiaoAddress').val();
            }
                // 获取列表数据
            http.request('shuibiao/list', 'get', param, function(res) {

            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">ETC展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">ETC编号</label>
                          <div class="layui-input-inline">
                              <input type="text" name="etcNumber" id="etcNumber" placeholder="ETC编号" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">ETC余额</label>
                          <div class="layui-input-inline">
                              <input type="text" name="etcMoneyStart" id="etcMoneyStart"  lay-skin="double"   placeholder="ETC余额开始" autocomplete="off" class="layui-input">
                              -
                              <input type="text" name="etcMoneyend"   id="etcMoneyend"    lay-skin="double|etcMoneyend"   placeholder="ETC余额结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">ETC编号所在地址</label>
                          <div class="layui-input-inline">
                              <input type="text" name="etcAddress" id="etcAddress" placeholder="ETC编号所在地址" autocomplete="off" class="layui-input">
                          </div>
                      </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 etcContent 字段去除无用字符 如果字段长度大于60  就截取到60
            etcContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        form.verify({
            ...
            ,etcMoneyend: function(value, item){ //value：表单的值、item：表单的DOM对象
                var etcMoneyStart = $("#etcMoneyStart").val();
                if(etcMoneyStart != null && etcMoneyStart != "" && etcMoneyStart != "null"){
                    var number = parseInt(value)- parseInt(etcMoneyStart);
                    if(number<0){
                        return 'ETC余额的结束值不能小于开始值';
                    }
                }
            }
        ...
        function pageList() {
            ...
            if (jquery('#etcNumber').val() != null && jquery('#etcNumber').val() != "" && jquery('#etcNumber').val() != "null" ) {
                param['etcNumber'] = jquery('#etcNumber').val();
            }
     
            if (jquery('#etcMoneyStart').val() != null && jquery('#etcMoneyStart').val() != "" && jquery('#etcMoneyStart').val() != "null" ) {
                param['etcMoneyStart'] = jquery('#etcMoneyStart').val();
            }
            if (jquery('#etcMoneyEnd').val() != null && jquery('#etcMoneyEnd').val() != "" && jquery('#etcMoneyEnd').val() != "null" ) {
                param['etcMoneyEnd'] = jquery('#etcMoneyEnd').val();
            }
     
            if (jquery('#etcAddress').val() != null && jquery('#etcAddress').val() != "" && jquery('#etcAddress').val() != "null" ) {
                param['etcAddress'] = jquery('#etcAddress').val();
            }
                // 获取列表数据
            http.request('etc/list', 'get', param, function(res) {

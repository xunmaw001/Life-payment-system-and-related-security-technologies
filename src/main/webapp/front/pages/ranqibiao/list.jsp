            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">燃气表展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">燃气表编号</label>
                          <div class="layui-input-inline">
                              <input type="text" name="ranqibiaoNumber" id="ranqibiaoNumber" placeholder="燃气表编号" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">燃气表余额</label>
                          <div class="layui-input-inline">
                              <input type="text" name="ranqibiaoMoneyStart" id="ranqibiaoMoneyStart"  lay-skin="double"   placeholder="燃气表余额开始" autocomplete="off" class="layui-input">
                              -
                              <input type="text" name="ranqibiaoMoneyend"   id="ranqibiaoMoneyend"    lay-skin="double|ranqibiaoMoneyend"   placeholder="燃气表余额结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">燃气表编号所在地址</label>
                          <div class="layui-input-inline">
                              <input type="text" name="ranqibiaoAddress" id="ranqibiaoAddress" placeholder="燃气表编号所在地址" autocomplete="off" class="layui-input">
                          </div>
                      </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 ranqibiaoContent 字段去除无用字符 如果字段长度大于60  就截取到60
            ranqibiaoContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        form.verify({
            ...
            ,ranqibiaoMoneyend: function(value, item){ //value：表单的值、item：表单的DOM对象
                var ranqibiaoMoneyStart = $("#ranqibiaoMoneyStart").val();
                if(ranqibiaoMoneyStart != null && ranqibiaoMoneyStart != "" && ranqibiaoMoneyStart != "null"){
                    var number = parseInt(value)- parseInt(ranqibiaoMoneyStart);
                    if(number<0){
                        return '燃气表余额的结束值不能小于开始值';
                    }
                }
            }
        ...
        function pageList() {
            ...
            if (jquery('#ranqibiaoNumber').val() != null && jquery('#ranqibiaoNumber').val() != "" && jquery('#ranqibiaoNumber').val() != "null" ) {
                param['ranqibiaoNumber'] = jquery('#ranqibiaoNumber').val();
            }
     
            if (jquery('#ranqibiaoMoneyStart').val() != null && jquery('#ranqibiaoMoneyStart').val() != "" && jquery('#ranqibiaoMoneyStart').val() != "null" ) {
                param['ranqibiaoMoneyStart'] = jquery('#ranqibiaoMoneyStart').val();
            }
            if (jquery('#ranqibiaoMoneyEnd').val() != null && jquery('#ranqibiaoMoneyEnd').val() != "" && jquery('#ranqibiaoMoneyEnd').val() != "null" ) {
                param['ranqibiaoMoneyEnd'] = jquery('#ranqibiaoMoneyEnd').val();
            }
     
            if (jquery('#ranqibiaoAddress').val() != null && jquery('#ranqibiaoAddress').val() != "" && jquery('#ranqibiaoAddress').val() != "null" ) {
                param['ranqibiaoAddress'] = jquery('#ranqibiaoAddress').val();
            }
                // 获取列表数据
            http.request('ranqibiao/list', 'get', param, function(res) {

            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">电表展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="dianbiaoNumber" id="dianbiaoNumber" placeholder="电表编号" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">电表余额</label>
                          <div class="layui-input-inline">
                              <input type="text" name="dianbiaoMoneyStart" id="dianbiaoMoneyStart"  lay-skin="double"   placeholder="电表余额开始" autocomplete="off" class="layui-input">
                              -
                              <input type="text" name="dianbiaoMoneyend"   id="dianbiaoMoneyend"    lay-skin="double|dianbiaoMoneyend"   placeholder="电表余额结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">电表编号所在地址</label>
                          <div class="layui-input-inline">
                              <input type="text" name="dianbiaoAddress" id="dianbiaoAddress" placeholder="电表编号所在地址" autocomplete="off" class="layui-input">
                          </div>
                      </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 dianbiaoContent 字段去除无用字符 如果字段长度大于60  就截取到60
            dianbiaoContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        form.verify({
            ...
            ,dianbiaoMoneyend: function(value, item){ //value：表单的值、item：表单的DOM对象
                var dianbiaoMoneyStart = $("#dianbiaoMoneyStart").val();
                if(dianbiaoMoneyStart != null && dianbiaoMoneyStart != "" && dianbiaoMoneyStart != "null"){
                    var number = parseInt(value)- parseInt(dianbiaoMoneyStart);
                    if(number<0){
                        return '电表余额的结束值不能小于开始值';
                    }
                }
            }
        ...
        function pageList() {
            ...
            if (jquery('#dianbiaoNumber').val() != null && jquery('#dianbiaoNumber').val() != "" && jquery('#dianbiaoNumber').val() != "null" ) {
                param['dianbiaoNumber'] = jquery('#dianbiaoNumber').val();
            }
     
            if (jquery('#dianbiaoMoneyStart').val() != null && jquery('#dianbiaoMoneyStart').val() != "" && jquery('#dianbiaoMoneyStart').val() != "null" ) {
                param['dianbiaoMoneyStart'] = jquery('#dianbiaoMoneyStart').val();
            }
            if (jquery('#dianbiaoMoneyEnd').val() != null && jquery('#dianbiaoMoneyEnd').val() != "" && jquery('#dianbiaoMoneyEnd').val() != "null" ) {
                param['dianbiaoMoneyEnd'] = jquery('#dianbiaoMoneyEnd').val();
            }
     
            if (jquery('#dianbiaoAddress').val() != null && jquery('#dianbiaoAddress').val() != "" && jquery('#dianbiaoAddress').val() != "null" ) {
                param['dianbiaoAddress'] = jquery('#dianbiaoAddress').val();
            }
                // 获取列表数据
            http.request('dianbiao/list', 'get', param, function(res) {

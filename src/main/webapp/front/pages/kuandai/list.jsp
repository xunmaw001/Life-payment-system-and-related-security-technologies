            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">宽带展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="kuandaiNumber" id="kuandaiNumber" placeholder="宽带编号" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">宽带余额</label>
                          <div class="layui-input-inline">
                              <input type="text" name="kuandaiMoneyStart" id="kuandaiMoneyStart"  lay-skin="double"   placeholder="宽带余额开始" autocomplete="off" class="layui-input">
                              -
                              <input type="text" name="kuandaiMoneyend"   id="kuandaiMoneyend"    lay-skin="double|kuandaiMoneyend"   placeholder="宽带余额结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">宽带编号所在地址</label>
                          <div class="layui-input-inline">
                              <input type="text" name="kuandaiAddress" id="kuandaiAddress" placeholder="宽带编号所在地址" autocomplete="off" class="layui-input">
                          </div>
                      </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 kuandaiContent 字段去除无用字符 如果字段长度大于60  就截取到60
            kuandaiContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        form.verify({
            ...
            ,kuandaiMoneyend: function(value, item){ //value：表单的值、item：表单的DOM对象
                var kuandaiMoneyStart = $("#kuandaiMoneyStart").val();
                if(kuandaiMoneyStart != null && kuandaiMoneyStart != "" && kuandaiMoneyStart != "null"){
                    var number = parseInt(value)- parseInt(kuandaiMoneyStart);
                    if(number<0){
                        return '宽带余额的结束值不能小于开始值';
                    }
                }
            }
        ...
        function pageList() {
            ...
            if (jquery('#kuandaiNumber').val() != null && jquery('#kuandaiNumber').val() != "" && jquery('#kuandaiNumber').val() != "null" ) {
                param['kuandaiNumber'] = jquery('#kuandaiNumber').val();
            }
     
            if (jquery('#kuandaiMoneyStart').val() != null && jquery('#kuandaiMoneyStart').val() != "" && jquery('#kuandaiMoneyStart').val() != "null" ) {
                param['kuandaiMoneyStart'] = jquery('#kuandaiMoneyStart').val();
            }
            if (jquery('#kuandaiMoneyEnd').val() != null && jquery('#kuandaiMoneyEnd').val() != "" && jquery('#kuandaiMoneyEnd').val() != "null" ) {
                param['kuandaiMoneyEnd'] = jquery('#kuandaiMoneyEnd').val();
            }
     
            if (jquery('#kuandaiAddress').val() != null && jquery('#kuandaiAddress').val() != "" && jquery('#kuandaiAddress').val() != "null" ) {
                param['kuandaiAddress'] = jquery('#kuandaiAddress').val();
            }
                // 获取列表数据
            http.request('kuandai/list', 'get', param, function(res) {

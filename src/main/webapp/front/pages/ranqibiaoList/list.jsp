            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">燃气表缴费记录展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="ranqibiaoListNewMoneyStart" id="ranqibiaoListNewMoneyStart"  lay-skin="double"   placeholder="缴费金额(元)开始" autocomplete="off" class="layui-input">
                              -
                              <input type="text" name="ranqibiaoListNewMoneyend"   id="ranqibiaoListNewMoneyend"    lay-skin="double|ranqibiaoListNewMoneyend"   placeholder="缴费金额(元)结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="insertTimeStart" id="insertTimeStart"  lay-skin="date"   placeholder="缴费时间开始" autocomplete="off" class="layui-input">
                              -
                              <input type="text" name="insertTimeend"   id="insertTimeend"    lay-skin="date"   placeholder="缴费时间结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="successTypes" id="successTypes" lay-filter="successTypes">
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 ranqibiaoListContent 字段去除无用字符 如果字段长度大于60  就截取到60
            ranqibiaoListContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        form.verify({
            ...
            ,ranqibiaoListNewMoneyend: function(value, item){ //value：表单的值、item：表单的DOM对象
                var ranqibiaoListNewMoneyStart = $("#ranqibiaoListNewMoneyStart").val();
                if(ranqibiaoListNewMoneyStart != null && ranqibiaoListNewMoneyStart != "" && ranqibiaoListNewMoneyStart != "null"){
                    var number = parseInt(value)- parseInt(ranqibiaoListNewMoneyStart);
                    if(number<0){
                        return '缴费金额(元)的结束值不能小于开始值';
                    }
                }
            }
        ...
        function pageList() {
            ...
            if (jquery('#ranqibiaoListNewMoneyStart').val() != null && jquery('#ranqibiaoListNewMoneyStart').val() != "" && jquery('#ranqibiaoListNewMoneyStart').val() != "null" ) {
                param['ranqibiaoListNewMoneyStart'] = jquery('#ranqibiaoListNewMoneyStart').val();
            }
            if (jquery('#ranqibiaoListNewMoneyEnd').val() != null && jquery('#ranqibiaoListNewMoneyEnd').val() != "" && jquery('#ranqibiaoListNewMoneyEnd').val() != "null" ) {
                param['ranqibiaoListNewMoneyEnd'] = jquery('#ranqibiaoListNewMoneyEnd').val();
            }
     
            if (jquery('#insertTimeStart').val() != null && jquery('#insertTimeStart').val() != "" && jquery('#insertTimeStart').val() != "null" ) {
                param['insertTimeStart'] = jquery('#insertTimeStart').val();
            }
            if (jquery('#insertTimeEnd').val() != null && jquery('#insertTimeEnd').val() != "" && jquery('#insertTimeEnd').val() != "null" ) {
                param['insertTimeEnd'] = jquery('#insertTimeEnd').val();
            }
     
            var successTypes = $("#successTypes").find("option:selected").val();
            if (successTypes != null && successTypes != "" && successTypes != "null") {
                param['successTypes'] = successTypes;
            }
                // 获取列表数据
            http.request('ranqibiaoList/list', 'get', param, function(res) {
        ...
        function dateTimePick(){
            ...
            */
        // 缴费时间的开始时间和结束时间的效验及格式
        var insertTimeStart = laydate.render({
            elem: '#insertTimeStart'
            ,type: 'datetime'
            ,done: function (value, dates) {
                insertTimeEnd.config.min={
                    year:dates.year,
        ...
        var insertTimeEnd = laydate.render({
            elem: '#insertTimeStart'
            ,type: 'datetime'
            ,done: function (value, dates) {
                insertTimeEnd.config.max={
                    year:dates.year,
                    month:dates.month-1,//关键
                    date: dates.date,
                    hours:dates.hours,
...
// 当前表的字典表
       ...
       function successTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=success_types", 'get', {}, function (res) {
               if(res.code == 0){

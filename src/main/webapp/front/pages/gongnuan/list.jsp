            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">供暖展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">供暖编号</label>
                          <div class="layui-input-inline">
                              <input type="text" name="gongnuanNumber" id="gongnuanNumber" placeholder="供暖编号" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">供暖余额</label>
                          <div class="layui-input-inline">
                              <input type="text" name="gongnuanMoneyStart" id="gongnuanMoneyStart"  lay-skin="double"   placeholder="供暖余额开始" autocomplete="off" class="layui-input">
                              -
                              <input type="text" name="gongnuanMoneyend"   id="gongnuanMoneyend"    lay-skin="double|gongnuanMoneyend"   placeholder="供暖余额结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">供暖编号所在地址</label>
                          <div class="layui-input-inline">
                              <input type="text" name="gongnuanAddress" id="gongnuanAddress" placeholder="供暖编号所在地址" autocomplete="off" class="layui-input">
                          </div>
                      </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 gongnuanContent 字段去除无用字符 如果字段长度大于60  就截取到60
            gongnuanContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        form.verify({
            ...
            ,gongnuanMoneyend: function(value, item){ //value：表单的值、item：表单的DOM对象
                var gongnuanMoneyStart = $("#gongnuanMoneyStart").val();
                if(gongnuanMoneyStart != null && gongnuanMoneyStart != "" && gongnuanMoneyStart != "null"){
                    var number = parseInt(value)- parseInt(gongnuanMoneyStart);
                    if(number<0){
                        return '供暖余额的结束值不能小于开始值';
                    }
                }
            }
        ...
        function pageList() {
            ...
            if (jquery('#gongnuanNumber').val() != null && jquery('#gongnuanNumber').val() != "" && jquery('#gongnuanNumber').val() != "null" ) {
                param['gongnuanNumber'] = jquery('#gongnuanNumber').val();
            }
     
            if (jquery('#gongnuanMoneyStart').val() != null && jquery('#gongnuanMoneyStart').val() != "" && jquery('#gongnuanMoneyStart').val() != "null" ) {
                param['gongnuanMoneyStart'] = jquery('#gongnuanMoneyStart').val();
            }
            if (jquery('#gongnuanMoneyEnd').val() != null && jquery('#gongnuanMoneyEnd').val() != "" && jquery('#gongnuanMoneyEnd').val() != "null" ) {
                param['gongnuanMoneyEnd'] = jquery('#gongnuanMoneyEnd').val();
            }
     
            if (jquery('#gongnuanAddress').val() != null && jquery('#gongnuanAddress').val() != "" && jquery('#gongnuanAddress').val() != "null" ) {
                param['gongnuanAddress'] = jquery('#gongnuanAddress').val();
            }
                // 获取列表数据
            http.request('gongnuan/list', 'get', param, function(res) {

            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">油卡展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">油卡编号</label>
                          <div class="layui-input-inline">
                              <input type="text" name="youkaNumber" id="youkaNumber" placeholder="油卡编号" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="youkaMoneyStart" id="youkaMoneyStart"  lay-skin="double"   placeholder="油卡余额开始" autocomplete="off" class="layui-input">
                              -
                              <input type="text" name="youkaMoneyend"   id="youkaMoneyend"    lay-skin="double|youkaMoneyend"   placeholder="油卡余额结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">油卡所属公司</label>
                          <div class="layui-input-inline">
                              <select name="youkaTypes" id="youkaTypes" lay-filter="youkaTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in youkaTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">油卡开户地址</label>
                          <div class="layui-input-inline">
                              <input type="text" name="youkaAddress" id="youkaAddress" placeholder="油卡开户地址" autocomplete="off" class="layui-input">
                          </div>
                      </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 youkaContent 字段去除无用字符 如果字段长度大于60  就截取到60
            youkaContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        form.verify({
            ...
            ,youkaMoneyend: function(value, item){ //value：表单的值、item：表单的DOM对象
                var youkaMoneyStart = $("#youkaMoneyStart").val();
                if(youkaMoneyStart != null && youkaMoneyStart != "" && youkaMoneyStart != "null"){
                    var number = parseInt(value)- parseInt(youkaMoneyStart);
                    if(number<0){
                        return '油卡余额的结束值不能小于开始值';
                    }
                }
            }
        ...
        function pageList() {
            ...
            if (jquery('#youkaNumber').val() != null && jquery('#youkaNumber').val() != "" && jquery('#youkaNumber').val() != "null" ) {
                param['youkaNumber'] = jquery('#youkaNumber').val();
            }
     
            if (jquery('#youkaMoneyStart').val() != null && jquery('#youkaMoneyStart').val() != "" && jquery('#youkaMoneyStart').val() != "null" ) {
                param['youkaMoneyStart'] = jquery('#youkaMoneyStart').val();
            }
            if (jquery('#youkaMoneyEnd').val() != null && jquery('#youkaMoneyEnd').val() != "" && jquery('#youkaMoneyEnd').val() != "null" ) {
                param['youkaMoneyEnd'] = jquery('#youkaMoneyEnd').val();
            }
     
            var youkaTypes = $("#youkaTypes").find("option:selected").val();
            if (youkaTypes != null && youkaTypes != "" && youkaTypes != "null") {
                param['youkaTypes'] = youkaTypes;
            }
     
            if (jquery('#youkaAddress').val() != null && jquery('#youkaAddress').val() != "" && jquery('#youkaAddress').val() != "null" ) {
                param['youkaAddress'] = jquery('#youkaAddress').val();
            }
                // 获取列表数据
            http.request('youka/list', 'get', param, function(res) {
...
// 当前表的字典表
       ...
       function youkaTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=youka_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.youkaTypesList = res.data.list;
               }
           });
       }
